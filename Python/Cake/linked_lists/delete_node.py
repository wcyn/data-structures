...
class Test(unittest.TestCase):
    ...
    def test_node_at_beginning(self):
        delete_linked_list_node_no_previous(self.first)
        actual = self.first.get_values()
        expected = [2, 3, 4]
        self.assertEqual(actual, expected)

    def test_node_in_middle(self):
        delete_linked_list_node_no_previous(self.second)
        actual = self.first.get_values()
        expected = [1, 3, 4]
        self.assertEqual(actual, expected)
    ...
    def test_node_at_end(self):
        with self.assertRaises(Exception):
            delete_linked_list_node_no_previous(self.fourth)

    def test_one_node_in_list(self):
        unique = Test.LinkedListNode(1)
        with self.assertRaises(Exception):
            delete_linked_list_node_no_previous(unique)
